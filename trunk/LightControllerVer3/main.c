...
//#define ConfigStr 			"[starts-with(UCPTname,\"Test/LON/N\")]"
//#define UnConfigStr 		"[UCPTitemStatus=\"IS_UNCONFIGURED\"][starts-with(UCPTname,\"Test/LON/N\")]"
//#define OffLineStr 			"[UCPTitemStatus=\"IS_OFFLINE\"][starts-with(UCPTname,\"Test/LON/N\")]"
        ...
        while (1){	
			...
			if(isNextSecond()){ //Moi giay mot lan doc			
		        ...
		        if(isSchedule(&currentTime1)){
					...
					//--------------------- SSS--------------------
					// if phase on 100% send sync realtime for DIM
					
					switch (checkCurrentTask()){
						case 3:			// on 100%
							RegBytes[0] = 0;
							RegBytes[1] = 2;
							RegBytes[2] = 0xFF;
							RegBytes[3] = currentTime1.second;	// second
							RegBytes[4] = currentTime1.minute;	// minute
							RegBytes[5] = currentTime1.hour;	// hour
							RegBytes[6] = currentTime1.day;	// day
							RegBytes[7] = currentTime1.date;	// date
							RegBytes[8] = currentTime1.month;	// month
							RegBytes[9] = currentTime1.year;	// year
							break;
						default:					// OFF erase Modbus reg
							for(i=0;i<30;i++){
								RegBytes[i] = 0;
							}
							break;
